...
def epsilon_greedy_policy(epsilon, q_s, state):
...
def train(env, episode_reward_list, eta, weights, v, alpha_scaling=None):
    low, high = env.observation_space.low, env.observation_space.high
    ...
    while not (done or truncated):
        ...
        action = epsilon_greedy_policy(epsilon, q_s, state)
        q_s_a = q_s[action]
        ...
        next_action = epsilon_greedy_policy(epsilon, q_s_prime, next_state)
        ...
        if HYPER_PARAM_MOMENTUM <= 0:
            # -- Vanilla SGD --
            weights = weights + alpha * delta_t * z
        else:
            v = HYPER_PARAM_MOMENTUM * v + alpha * delta_t * z
            # -- SGD with Nesterov Acceleration / SGD with Momentum --
            weights = weights + ((HYPER_PARAM_MOMENTUM * v + alpha * delta_t * z) if HYPER_PARAM_NESTROV else v)
...
for i in range(HYPER_PARAM_TRAIN_EPISODE):
    if (((i+1) % EVAL_PERIOD) == 0) or (i == (HYPER_PARAM_TRAIN_EPISODE-1)):
       avg_reward, conf = evaluate(env, NUM_EVAL_EPISODES, ETA, weights)
    ...
    episode_reward_list, weights = train(env, episode_reward_list, ETA, weights, v, alpha_scaling)    
    
print(best_avg_reward, best_conf)
