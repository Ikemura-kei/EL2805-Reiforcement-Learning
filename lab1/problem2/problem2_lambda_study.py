...
def train(env, episode_reward_list, eta, weights, v, alpha_scaling=None, alpha=2e-3, show=False, lambda_=0.99):
    low, high = env.observation_space.low, env.observation_space.high
...
HYPER_PARAM_LAMBDA = np.linspace(0.05, 1., 300)
HYPER_PARAM_GAMMA = 1
...
best_perfs = []
for lambda_ in HYPER_PARAM_LAMBDA:
    ...
    for i in range(HYPER_PARAM_TRAIN_EPISODE):
        ...
        episode_reward_list, weights = train(env, episode_reward_list, ETA, weights, v, alpha_scaling, lambda_=lambda_, alpha=HYPER_PARAM_ALPHA)    
    
    print([lambda_, best_avg_reward, best_conf])
    best_perfs.append([lambda_, best_avg_reward, best_conf])
    
np.save('perf_vs_lambda.npy', best_perfs)
