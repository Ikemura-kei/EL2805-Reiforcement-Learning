...
for w_std in STDS:
    ...
    env.reset(seed=seed)
    weights = np.zeros((N_ACTIONS, N_BASIS)) + np.random.normal(0, w_std, (N_ACTIONS, N_BASIS))
    ...
    for i in range(HYPER_PARAM_TRAIN_EPISODE):
        ...
        episode_reward_list, weights = train(env, episode_reward_list, ETA, weights, v, alpha_scaling, alpha=HYPER_PARAM_ALPHA)    
    
    print([w_std, best_avg_reward, best_conf])
    best_perfs.append([w_std, best_avg_reward, best_conf])
    
np.save('perf_vs_init_q_std.npy', best_perfs)
